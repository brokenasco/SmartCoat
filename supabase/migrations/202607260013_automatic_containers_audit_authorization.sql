-- Allow trusted draft audit events and derive paint purchases server-side.
...
alter table public.estimates
  add column if not exists other_direct_materials_cents bigint not null default 0;
alter table public.estimates drop constraint if exists estimates_other_direct_materials_check;
alter table public.estimates add constraint estimates_other_direct_materials_check
  check(other_direct_materials_cents>=0);
...
begin
  if new.status='draft' and jsonb_typeof(new.draft_payload->'rooms')='array'
    ...
    new.other_direct_materials_cents:=greatest(
      round(coalesce(nullif(new.draft_payload->>'otherDirectMaterialsDollars','')::numeric,0)*100),
      0
    );
  end if;
  return new;
...
declare room jsonb; opening jsonb; l numeric; w numeric; h numeric; gross numeric; deducted numeric;
  ...
  other_materials bigint; brand text; color_code text;
begin
  ...
  other_materials:=greatest(round(coalesce(nullif(payload->>'otherDirectMaterialsDollars','')::numeric,0)*100),0)::bigint;
  ...
  project_direct:=project_direct+other_materials;
