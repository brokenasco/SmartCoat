...
export default async function Dashboard({ searchParams }: { searchParams: Promise<{ tutorial?: string }> }) {
  ...
  return <main className="min-h-screen">
    ...
    <div className="mx-auto max-w-7xl px-5 py-8">
      <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm text-muted">Operations overview</p><h1 className="text-3xl font-semibold tracking-tight">Good work starts with a clear number.</h1></div><div className="flex flex-wrap gap-3"><DashboardTutorial initiallyActive={tutorialActive} plan={tutorialPlanFromStatus(entitlement?.status)}/><Link href="/dashboard/estimates" className="inline-flex min-h-12 items-center rounded-lg border border-brand px-5 font-semibold text-brand">Drafts & Approved</Link><Link data-tutorial-id="new-estimate" href={tutorialActive ? "/dashboard/estimates/new?tutorial=1" : "/dashboard/estimates/new"} className="inline-flex min-h-12 items-center gap-2 rounded-lg bg-brand px-5 font-semibold text-white"><Plus size={18}/>New estimate</Link></div></div>
