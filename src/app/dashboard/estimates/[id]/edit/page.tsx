...
export default async function EditEstimate({params}:{params:Promise<{id:string}>}){
  ...
  const {data:membership}=await supabase.from("company_memberships").select("company_id").eq("user_id",user.id).eq("status","active").limit(1).single(); if(!membership) redirect("/dashboard");
  const [{data:estimate},{data:brands}]=await Promise.all([
    supabase.from("estimates").select("id,title,status,draft_payload").eq("id",id).eq("company_id",membership.company_id).single(),
