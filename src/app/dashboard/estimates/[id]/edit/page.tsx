...
export default async function EditEstimate({params}:{params:Promise<{id:string}>}){
  ...
  const {data:estimate}=await supabase.from("estimates").select("id,title,status,draft_payload").eq("id",id).eq("company_id",membership.company_id).is("deleted_at",null).single();
